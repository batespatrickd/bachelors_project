# Code debugged and modified with the help of OpenAI. (2025). ChatGPT (June 11 version) [Large language model]. https://chat.openai.com/
...
def generate_random_coordinates(N, center, radius): ######### original ###########
    """
...
# def generate_random_coordinates(N, center, radius):   ######### uniform in area ##########
#     """
#     Generates N random 2D cartesian coords uniformly across the area of a circle.
#     """
#     # Randomly sample radial distances with a square-root distribution
#     r = np.sqrt(np.random.rand(N)) * radius  # sqrt to account for area
#     # Randomly sample angles uniformly from 0 to 2pi
...
#     # Stack into an (N, 2) array for 2D coordinates
...
class SpectrumGenerator():     ######## saves tau data and a .png
    """
    ...
    ######### attempt to save velocity and tau #########
    def plot_spectrum(self, filename=None, component=False,
                    velocity=True, annotate=True, mean=False, cloud_vels=False,
                    ray_plot=False):
        """
        Plot spectrum in angstroms or km/s to filename and save the data to a single HDF5 file
        with ray number and delimiters.
        """
        
        # File paths
        tau_filename = 'all_tau_data.h5'  # Unified HDF5 file for all tau data

        # Determine x and y fields for spectrum plotting
        x_field = self.velocity_field if velocity else self.lambda_field
        # y_field = self.flux_field  # Unused in saving process

        # Save tau data
        if hasattr(self, 'tau_field'):
            with h5py.File(tau_filename, 'a') as hf:
                # Ensure tau_field is 1D
                tau_values = self.tau_field.flatten()
                x_values = x_field.flatten()

                if 'tau_data' not in hf:
                    hf.create_dataset('tau_data', data=tau_values, maxshape=(None,))
                    hf.create_dataset('ray_numbers', data=np.full_like(tau_values, self.i + 1, dtype=int), maxshape=(None,))
                    hf.create_dataset('x_data', data=x_values, maxshape=(None,))
                else:
                    # Resize datasets correctly
                    hf['tau_data'].resize(hf['tau_data'].shape[0] + tau_values.shape[0] + 1, axis=0)
                    hf['tau_data'][-(tau_values.shape[0] + 1):-1] = tau_values  # Append tau values
                    hf['tau_data'][-1] = np.nan  # Separator

                    # Append ray numbers
                    ray_numbers = np.full_like(tau_values, self.i + 1, dtype=int)
                    hf['ray_numbers'].resize(hf['ray_numbers'].shape[0] + ray_numbers.shape[0] + 1, axis=0)
                    hf['ray_numbers'][-(ray_numbers.shape[0] + 1):-1] = ray_numbers
                    hf['ray_numbers'][-1] = -1  # Separator

                    # Append corresponding x_field values
                    hf['x_data'].resize(hf['x_data'].shape[0] + x_values.shape[0] + 1, axis=0)
                    hf['x_data'][-(x_values.shape[0] + 1):-1] = x_values
                    hf['x_data'][-1] = np.nan  # Separator


# ##### saves tau for all specturm ######
#     def plot_spectrum(self, filename=None, component=False,
#                     velocity=True, annotate=True, mean=False, cloud_vels=False,
#                     ray_plot=False):
#         """
...
#         # File paths
#         tau_filename = 'all_tau_data.h5'  # Unified HDF5 file for all tau data
#         # csv_filename = 'all_tau_data.csv'  # CSV file for tau values with ray number

#         # Determine x and y fields for spectrum plotting
#         # x_field = self.velocity_field if velocity else self.lambda_field
#         # y_field = self.flux_field

#         # # Save spectrum data to CSV (individual file per spectrum)
#         # spectrum_filename = f'spectrum_{self.i + 1:04d}_data.csv'
#         # with open(spectrum_filename, mode='a', newline='') as file:
#         #     writer = csv.writer(file)
#         #     if file.tell() == 0:  # If file is empty, write the header
#         #         writer.writerow(['X', 'Relative Flux'])
#         #     writer.writerows(zip(x_field, y_field))
...
#             # Also save tau data to CSV
#             # with open(csv_filename, 'a', newline='') as file:
#             #     writer = csv.writer(file)
#             #     if file.tell() == 0:
#             #         writer.writerow(["Ray Number", "Tau Value"])  # Write header if file is new

#             #     for tau in self.tau_field:
#             #         writer.writerow([self.i + 1, tau])

#             #     # Add delimiter row between spectra
#             #     writer.writerow(["---", "---"])
...
# #         flux_field = self.flux_mean_field if mean else self.flux_field
# #         EW_val = self.mean_EW if mean else self.rays.EWs[self.i]
# #         column_density = self.sum_column_density if mean else self.rays.column_densities[self.i]
# #         n_intersections = self.sum_intersections if mean else self.rays.n_clouds[self.i]

# #         # ax.set_xlim(x_field.min(), x_field.max())
# #         # ax.set_xlabel('Velocity [km/s]' if velocity else '$\lambda$ [Angstrom]')
# #         # ax.step(x_field[::2], flux_field[::2], 'k', where='mid', linewidth=2)
# #         # ax.set_ylim(0, 1.2)
...
# #         # # Save spectrum image
# #         # filename = filename or (f'spectrum_{self.i + 1:04d}.png' if not mean else 'spectrum_mean.png')
# #         # plt.savefig(filename)
# #         # plt.close()

# #         # print(f"Spectrum saved to {filename}")
# #         # print(f"Spectrum data saved to {spectrum_filename}")
# #         print(f"Tau data saved to {tau_filename}")  # Always saves tau to the same .h5 file now

#####################################################################################

    # # Used OpenAI. 20.2.25 ChatGPT (GPT-4 version) for rewriting function to save data as .csv
    # ######### save tau, and plot BELOW ##########
    # def plot_spectrum(self, filename=None, component=False,
    #               velocity=True, annotate=True, mean=False, cloud_vels=False,
    #               ray_plot=False):
    #     """
    #     Plot spectrum in angstroms or km/s to filename and save the data to CSV files.
    #     Also plots tau as a function of velocity or wavelength.
    #     """

    #     # Create filenames for saving spectrum and tau data
    #     if mean:
    #         spectrum_filename = 'spectrum_mean_data.csv'
    #         tau_filename = 'tau_mean_data.csv'
    #     else:
    #         spectrum_filename = f'spectrum_{self.i + 1:04d}_data.csv'
    #         tau_filename = f'tau_{self.i + 1:04d}_data.csv'
    ...
    #         if file.tell() == 0:  # If file is empty, write the header
    #             writer.writerow(['X', 'Relative Flux'])  
    #         writer.writerows(zip(x_field, self.flux_field))  

    #     # Save tau data to CSV
    #     if hasattr(self, 'tau_field'):
    #         with open(tau_filename, mode='a', newline='') as file:
    #             writer = csv.writer(file)
    #             if file.tell() == 0:
    #                 writer.writerow(['X', 'Tau'])  
    #             writer.writerows(zip(x_field, self.tau_field))  

    #     # Plot Relative Flux spectrum
    #     fig, ax = plt.subplots()
    #     fig.set_size_inches(5.5, 4.5)

    #     ax.step(x_field[::2], self.flux_field[::2], 'k', where='mid', linewidth=2)
    #     ax.set_ylim(0, 1.2)
    #     ax.set_ylabel('Relative Flux')
    #     ax.xaxis.set_major_locator(MultipleLocator(50))
    #     ax.xaxis.set_minor_locator(ticker.AutoMinorLocator())
    ...
    #         ax.set_xlim(-110, 110)
    #     else:
    #         ax.set_xlabel(r'$\lambda$ [Angstrom]')  # Fixed invalid escape sequence

    #     if annotate:
    #         text1 = f'Number of Cloudlets = {self.rays.n_clouds[self.i]:.2f}'
    #         text2 = f'Column Density = {self.rays.column_densities[self.i]:3.1g} ' + r'${\rm cm^{-2}}$'  # Fixed LaTeX
    #         text3 = f'EW = {self.rays.EWs[self.i]:4.2f} ' + r'${\rm \AA}$'
    #         ax.text(0.02, 0.95, text1, size='medium', weight='bold', color='black', transform=ax.transAxes)
    #         ax.text(0.02, 0.90, text2, size='medium', weight='bold', color='black', transform=ax.transAxes)
    #         ax.text(0.02, 0.85, text3, size='medium', weight='bold', color='black', transform=ax.transAxes)

    #     plt.tight_layout()
    #     spectrum_plot_filename = filename or f'spectrum_{self.i + 1:04d}.png'
    #     plt.savefig(spectrum_plot_filename)
    #     plt.close()

    #     print(f"Spectrum saved to {spectrum_plot_filename}")
    #     print(f"Spectrum data saved to {spectrum_filename}")

    #     # Plot Tau as a function of X (velocity or wavelength)
    #     if hasattr(self, 'tau_field'):
    #         fig, ax = plt.subplots()
    #         fig.set_size_inches(5.5, 4.5)

    #         ax.step(x_field[::2], self.tau_field[::2], 'r', where='mid', linewidth=2)
    #         ax.set_ylabel(r'$\tau$')  # Properly formatted tau label
    #         ax.xaxis.set_major_locator(MultipleLocator(50))
    #         ax.xaxis.set_minor_locator(ticker.AutoMinorLocator())
    ...
    #         plt.tight_layout()
    #         tau_plot_filename = f'tau_{self.i + 1:04d}.png'
    #         plt.savefig(tau_plot_filename)
    #         plt.close()

    #         print(f"Tau plot saved to {tau_plot_filename}")
    #         print(f"Tau data saved to {tau_filename}")

    
    # ######## save tau but not plot below ##########
    # def plot_spectrum(self, filename=None, component=False,
    #               velocity=True, annotate=True, mean=False, cloud_vels=False,
    #               ray_plot=False):
    #     """
    ...
    #     # Save tau data to CSV if tau_field exists
    #     if hasattr(self, 'tau_field'):
    #         with open(tau_filename, mode='a', newline='') as file:
    #             writer = csv.writer(file)
    #             if file.tell() == 0:
    ...
    #         else:
    #             filename = f'spectrum_{self.i + 1:04d}.png'
    #     plt.savefig(filename)
    #     plt.close()

    #     print(f"Spectrum saved to {filename}")
    #     print(f"Spectrum data saved to {spectrum_filename}")
    #     print(f"Tau data saved to {tau_filename}")  # Always saves tau now


    # def plot_spectrum(self, filename=None, component=False, ##### original ######
    #                 velocity=True, annotate=True, mean=False, cloud_vels=False,
    #                 ray_plot=False):
    #     """
    #     Plot spectrum in angstroms or km/s to filename and save the data to a CSV file
    #     """
    #     # Create a filename for saving the data
    #     if mean:
    #         data_filename = 'spectrum_mean_data.csv'
    #     else:
    #         #data_filename = f'spectrum_{self.i:04d}_data.csv'
    #         data_filename = f'spectrum_{self.i +1:04d}_data.csv'
    ...
    #         writer = csv.writer(file)
    #         if file.tell() == 0:  # If file is empty, write the header
    #             writer.writerow(['X', 'Relative Flux'])  # Adjust headers as needed
    #         writer.writerows(zip(x_field, y_field))  # Write the data
    ...
    #     # Existing plotting logic (annotations, flux field plotting, etc.)...
    ...
    #             filename = 'spectrum_mean.png'
    #         else:
    #             #filename = f'spectrum_{self.i:04d}.png'
    #             filename = f'spectrum_{self.i + 1:04d}.png'
